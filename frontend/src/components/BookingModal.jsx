...
const EVENT_TYPES = ["Nikkah", "Walima", "Birthday", "Conference", "Anniversary", "Other"];
...
export default function BookingModal({ hall: preselectedHall, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    eventType: "Walima", eventDate: "",
    guests: 2, paymentMethod: "", transactionId: "",
    specialRequests: "",
  });
  ...
  useEffect(() => {
    ...
    api.get(`/bookings/slots/${form.hallId}`).then((res) => {
  ...
  const validate = (s) => {
    ...
    if (s === 2) {
      ...
      if (form.eventDate && bookedDates.includes(form.eventDate)) e.eventDate = "Already booked on this date";
    }
    if (s === 3) {
              ...
              <div key={i} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center">
                  <div
                    ...
                    style={{
                      background: i + 1 < step ? "#7c3aed" : i + 1 === step ? "rgba(124,58,237,0.3)" : "rgba(255,255,255,0.06)",
        ...
        <div className="px-6 pb-2">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <Field label="Full Name *" error={errors.clientName}>
                  <input className={inputClass(errors.clientName)} placeholder="Ahmed Khan"
                    value={form.clientName} onChange={(e) => set("clientName", e.target.value)} />
                </Field>
                <Field label="Phone *" error={errors.clientPhone}>
                  <input className={inputClass(errors.clientPhone)} placeholder="+92 300 1234567"
                    value={form.clientPhone} onChange={(e) => set("clientPhone", e.target.value)} />
                </Field>
              </div>
              <Field label="Email" error={errors.clientEmail}>
                <input className={inputClass(errors.clientEmail)} type="email" placeholder="ahmed@email.com"
                  value={form.clientEmail} onChange={(e) => set("clientEmail", e.target.value)} />
              </Field>
              <Field label="Special Requests">
                <input className={inputClass()} placeholder="Decoration style, dietary needs..."
          ...
          {step === 2 && (
            <div className="space-y-4">
              <Field label="Select Hall *" error={errors.hallId}>
                <select className={inputClass(errors.hallId)} value={form.hallId}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Event Type">
                  <select className={inputClass()} value={form.eventType}
                    onChange={(e) => set("eventType", e.target.value)}
                    style={{ background: "rgba(255,255,255,0.05)" }}>
                    {EVENT_TYPES.map((t) => <option key={t} style={{ background: "#1a1035" }}>{t}</option>)}
                  </select>
                </Field>
                <Field label="No. of Guests">
                  <input className={inputClass()} type="number" min="1" value={form.guests}
                    onChange={(e) => set("guests", parseInt(e.target.value) || 1)} />
                </Field>
              </div>

              <Field label="Event Date *" error={errors.eventDate}>
                <input className={inputClass(errors.eventDate)} type="date" min={today}
              ...
              {bookedDates.length > 0 && (
                <div className="text-xs rounded-lg p-2.5" style={{ background: "rgba(239,68,68,0.08)", color: "#f87171" }}>
                  ⚠ Already booked on: {bookedDates.join(", ")}
                </div>
              )}
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              {form.paymentMethod && form.paymentMethod !== "Cash" && (
                <Field label={`${form.paymentMethod} Transaction ID *`} error={errors.transactionId}>
                  <div className="flex gap-2">
                    <input className={`${inputClass(errors.transactionId)} flex-1`}
                      placeholder="Enter reference / transaction ID"
                      value={form.transactionId} onChange={(e) => set("transactionId", e.target.value)} />
                    <button type="button" onClick={handleVerify} disabled={verifying || payVerified}
              ...
              {errors.submit && (
                <p className="text-sm text-center rounded-lg p-2"
          ...
          {step === 4 && (
            <div className="text-center py-2">
              ...
              <div className="rounded-xl p-4 mb-4"
                ...
                <p className="text-white font-bold text-2xl tracking-widest" style={{ fontFamily: "'Playfair Display',serif" }}>
                  {bookingRef}
                </p>
              </div>
              {[
