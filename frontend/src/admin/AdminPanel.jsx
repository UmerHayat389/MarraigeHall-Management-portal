  ...
  return (
    <div className="admin-wrap">
      ...
      <div className="admin-topbar">
        ...
        <button
          onClick={logout}
          style={{ ...btnDanger, fontSize: "0.72rem", padding: "0.32rem 0.75rem" }}
        >
          Logout
        </button>
      </div>
      ...
      <div className="admin-layout">
        ...
        <aside className={`admin-sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          <div style={{ borderTop: "1px solid rgba(139,92,246,0.12)", paddingTop: "1rem" }}>
            ...
            <button
              onClick={logout}
              style={{ ...btnDanger, width: "100%", textAlign: "center", padding: "0.62rem", fontSize: "0.82rem" }}
            >
              Sign Out
            </button>
          </div>
        ...
        <main className="admin-main">
          {activeTab === "dashboard" && (
            <DashboardTab stats={stats} bookings={allBookings} switchTab={switchTab} />
          )}
          {activeTab === "halls" && (
            <HallsTab toast={showToast} />
          )}
          {activeTab === "bookings" && (
            <BookingsTab toast={showToast} />
          )}
          {activeTab === "dishes" && (
      ...
      {toast && (
        <AdminToast msg={toast.msg} type={toast.type} onDone={() => setToast(null)} />
      )}
    </div>
  );
}