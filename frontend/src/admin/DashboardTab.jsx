    ...
    <div style={{ display:"flex", alignItems:"flex-end", gap:"5px", height:72 }}>
      {data.map((d,i) => (
        <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3, minWidth:0 }}>
          ...
          <span style={{ color:"rgba(255,255,255,0.28)", fontSize:"0.6rem", textAlign:"center", lineHeight:1.2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", width:"100%" }}>{d.label}</span>
        </div>
      ))}
    </div>
  );
...
export default function DashboardTab({ stats, bookings, switchTab }) {
  ...
  const statCards = [
    { label:"Total Halls",    value:stats.halls,    icon:"🏛️", color:"#9333ea", sub:"venues" },
  ...
  return (
    <div>
      <div style={{ marginBottom:"1.75rem" }}>
        <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(1.5rem,4vw,2rem)", fontWeight:400, margin:"0 0 6px", color:"white" }}>
          Welcome to <em style={{ color:"#9333ea", fontStyle:"italic" }}>Admin</em>
        </h1>
        ...
        {statCards.map(s => (
          <div key={s.label} className="stat-card">
            ...
            <div style={{ minWidth:0 }}>
              <p style={{ color:"rgba(255,255,255,0.35)", fontSize:"0.68rem", letterSpacing:"0.1em", textTransform:"uppercase", margin:"0 0 3px" }}>{s.label}</p>
              <p style={{ color:"white", fontSize:"1.65rem", fontWeight:700, fontFamily:"'Cormorant Garamond',serif", margin:0, lineHeight:1 }}>{s.value}</p>
              <p style={{ color:"rgba(255,255,255,0.22)", fontSize:"0.68rem", margin:"3px 0 0" }}>{s.sub}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="charts-row" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem", marginBottom:"1.25rem" }}>
        {/* Bar chart */}
        <div style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.015))", border:"1px solid rgba(139,92,246,0.13)", borderRadius:"18px", padding:"1.35rem" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1rem" }}>
            <div>
              <p style={{ color:"rgba(255,255,255,0.55)", fontSize:"0.78rem", fontWeight:600, margin:0 }}>Bookings — Last 7 Days</p>
              <p style={{ color:"rgba(255,255,255,0.25)", fontSize:"0.68rem", margin:"2px 0 0" }}>New bookings per day</p>
            </div>
            <span style={{ background:"rgba(147,51,234,0.15)", border:"1px solid rgba(147,51,234,0.28)", borderRadius:"999px", padding:"2px 9px", color:"#c084fc", fontSize:"0.7rem", fontWeight:600 }}>
              {last7.reduce((s,d)=>s+d.value,0)} total
            </span>
          </div>
          <MiniBarChart data={last7} color="#9333ea" />
        </div>

        {/* Status breakdown */}
        <div style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.015))", border:"1px solid rgba(139,92,246,0.13)", borderRadius:"18px", padding:"1.35rem" }}>
          <p style={{ color:"rgba(255,255,255,0.55)", fontSize:"0.78rem", fontWeight:600, margin:"0 0 1.1rem" }}>Status Breakdown</p>
          {[["Confirmed","#10b981",confirmed],["Pending","#f59e0b",pending],["Cancelled","#ef4444",cancelled]].map(([l,c,v])=>(
            <div key={l} style={{ marginBottom:"0.85rem" }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"5px" }}>
                <span style={{ color:"rgba(255,255,255,0.5)", fontSize:"0.8rem", display:"flex", alignItems:"center", gap:"0.4rem" }}>
                  <span style={{ width:7, height:7, borderRadius:"50%", background:c, display:"inline-block" }} />{l}
                </span>
                <span style={{ color:c, fontWeight:700, fontSize:"0.8rem" }}>{v} <span style={{ color:"rgba(255,255,255,0.25)", fontWeight:400 }}>({Math.round((v/total)*100)}%)</span></span>
              </div>
      ...
      <div className="dash-bottom" style={{ display:"grid", gridTemplateColumns:"1fr 200px", gap:"1rem" }}>
        {/* Recent bookings */}
        <div style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.015))", border:"1px solid rgba(139,92,246,0.13)", borderRadius:"18px", padding:"1.35rem" }}>
          <p style={{ color:"rgba(255,255,255,0.55)", fontSize:"0.78rem", fontWeight:600, margin:"0 0 1rem", textTransform:"uppercase", letterSpacing:"0.08em" }}>Recent Bookings</p>
          {recent.length===0
            ? <p style={{ color:"rgba(255,255,255,0.22)", fontSize:"0.84rem" }}>No bookings yet</p>
            : recent.map(b=>(
              <div key={b._id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"0.55rem 0", borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
                <div style={{ minWidth:0 }}>
        ...
        {/* Quick actions */}
        <div style={{ background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.015))", border:"1px solid rgba(139,92,246,0.13)", borderRadius:"18px", padding:"1.35rem" }}>
          <p style={{ color:"rgba(255,255,255,0.55)", fontSize:"0.78rem", fontWeight:600, margin:"0 0 1rem", textTransform:"uppercase", letterSpacing:"0.08em" }}>Quick Actions</p>
          <div style={{ display:"flex", flexDirection:"column", gap:"0.55rem" }}>
            {[
              ...
              <button key={tab} onClick={()=>switchTab(tab)} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(139,92,246,0.16)", borderRadius:"10px", color:"rgba(255,255,255,0.65)", fontSize:"0.82rem", cursor:"pointer", padding:"0.6rem 0.85rem", textAlign:"left", display:"flex", alignItems:"center", gap:"0.55rem", fontFamily:"'Plus Jakarta Sans',sans-serif", transition:"all 0.15s" }}
                onMouseEnter={e=>{e.currentTarget.style.background="rgba(109,40,217,0.18)";e.currentTarget.style.color="white";e.currentTarget.style.borderColor="rgba(147,51,234,0.35)";}}
                onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.04)";e.currentTarget.style.color="rgba(255,255,255,0.65)";e.currentTarget.style.borderColor="rgba(139,92,246,0.16)";}}>
